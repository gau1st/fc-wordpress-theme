...
function _247themes_sc_portfolio_catlist($attr) {
    ...
    foreach (explode(",", $categories) as $c) {
        ...
        $cid = uniqid();
        $parents[] = '<a href="" class="portfolio_parent type" category="'.$cat->term_id.'">'.$cat->name.'</a>';
        
        $thispanel = '
            <div class="portfolio_category container3" category="'.$cat->term_id.'" style="display:none;"><ul>';
        foreach ($cats as $child) {
            $thispanel.= '
                <li><a href="" category="'.$child->term_id.'">'.$child->name.'</a></li>
            ';
        }
        $thispanel.= '
            </ul></div>';
    ...
    $ans.= '<div class="portfolios container3"></div><div class="portfolio_desc container3" style="width:190px;"></div></div>';
...
function _247themes_ajax_portfolios() {
    ...
    echo '<ul>';
    foreach ($tmp as $p) {
        echo '<li><a href="" post_id="'.$p->ID.'" post_desc="'.$p->post_content.'">'.$p->post_title.'</a></li>';
    }
    echo '</ul>';
    die();
}
...
function _247themes_ajax_portfolio_images() {
    ...
    $(document).ready(function(){
        $(".slides").slides({
            preload: true,
            preloadImage: "'.get_bloginfo('template_url').'/images/loading/loading21.gif",
            play: 4000,
            pause: 2500,
            hoverPause: true
		});
